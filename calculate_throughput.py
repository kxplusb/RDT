...
def RDT_start_test(port=12345):
    sender = Process(target=RDT_send_file, args=(("localhost", port),("localhost", port)))
    receiver = Process(target=RDT_receive_file, args=(("localhost", port),("localhost", port)))
...
def RDT_receive_file(source_address, target_address, file_path ='./transmit.txt'):
    """
    ...
    data = time.time()*1000
    rcvd_data = server_sock.recv()
    spend_time = float(time.time() * 1000) - float(data)
    print(f"RDT throught put : {spend_time / 1000} s")
    sock.close()
    server_sock.close()

    with open(file_path, 'w') as file:
        file.write(rcvd_data)
